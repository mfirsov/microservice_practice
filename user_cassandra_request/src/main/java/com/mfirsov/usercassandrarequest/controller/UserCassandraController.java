...
import com.mfirsov.usercassandrarequest.util.BankAccountInfoEntityModelConverter;
...
//        Mono<BankAccountInfo> bankAccountInfo = customCassandraRepository.findBankAccountInfoByUuid(parsedUuid);
        return bankAccountInfoService.getBankAccountInfoByUuid(parsedUuid)
                .map(BankAccountInfoEntityModelConverter::from)
                .map(BankAccountInfoResponse::new);
    }
